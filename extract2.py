...
Given the following medical note, extract the following information, make bullet points inside each bullet point if needed: 
- visit motivation
- admission
- patient information
...
treatments_template = """
Given the following medical note, extract the 'treatments' information, make bullet points inside each bullet point if needed: 
...
treatment_prediction_template = """
Based on the extracted general information from the patient's medical note, predict the appropriate medical treatments, make bullet points inside each bullet point if needed. 

Extracted General Information:
{general_info}
...
def extract_general_info(note):
    ...
    prompt = PromptTemplate(template=general_info_template, input_variables=["note"])
    sequence = RunnableSequence(prompt | llm | parser)
    response = sequence.invoke({"note": note})
    return response
...
def extract_treatments(note):
    parser = StrOutputParser()
    prompt = PromptTemplate(template=treatments_template, input_variables=["note"])
    sequence = RunnableSequence(prompt | llm | parser)
    response = sequence.invoke({"note": note})
    return response

# Function to predict treatments based on general information
def predict_treatments(general_info):
    parser = StrOutputParser()
    prompt = PromptTemplate(template=treatment_prediction_template, input_variables=["general_info"])
    sequence = RunnableSequence(prompt | llm | parser)
    response = sequence.invoke({"general_info": general_info})
    return response
