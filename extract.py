...
Based on the extracted general information from the patient's medical note, predict the appropriate medical treatments and format them as JSON.
...
def predict_treatments(general_info):
    ...
    sequence = RunnableSequence(prompt | llm | parser)
    response = sequence.invoke({"general_info": json.dumps(general_info, indent=4)})
    return json.loads(response)
...
print("\nPredicted Treatments:\n", json.dumps(predicted_treatments, indent=4))
